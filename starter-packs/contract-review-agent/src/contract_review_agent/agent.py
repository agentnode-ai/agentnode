"""contract_review_agent — AgentNode agent (ANP v0.2)

Contract Review Agent: Analyze legal contracts, flag risky clauses, compare against templates, and suggest amendments.
"""
...
class ContractReviewAgent:
    """
    Review a legal contract by extracting key clauses, identifying risks and unusual terms, comparing against standard templates, and suggesting amendments.
...
async def run(goal: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    """
    ctx = context or {}
    agent = ContractReviewAgent(api_key=ctx.get("api_key"))
