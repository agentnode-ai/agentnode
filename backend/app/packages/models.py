...
class Package(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    pricing_model = Column(
        Enum(
            "free", "one_time", "subscription", name="pricing_model", create_type=False
        ),
        nullable=False,
        default="free",
    )
    ...
    latest_version = relationship(
        "PackageVersion", foreign_keys=[latest_version_id], post_update=True
    )
...
class PackageVersion(Base, UUIDPrimaryKeyMixin):
    ...
    latest_verification_result_id = Column(
        ...
        ForeignKey(
            ...
            ondelete="SET NULL",
            use_alter=True,
            name="fk_latest_verification_result",
        ),
        nullable=True,
    )
    verification_run_count = Column(
    ...
    execution_success_count = Column(
        Integer, nullable=False, default=0, server_default="0"
    )
    ...
    package = relationship(
        "Package", back_populates="versions", foreign_keys=[package_id]
    )
    capabilities = relationship(
        "Capability", back_populates="package_version", cascade="all, delete-orphan"
    )
    tags = relationship("PackageTag", cascade="all, delete-orphan")
    categories = relationship("PackageCategory", cascade="all, delete-orphan")
    compatibility_rules = relationship(
        "CompatibilityRule", cascade="all, delete-orphan"
    )
    dependencies = relationship("Dependency", cascade="all, delete-orphan")
    permissions = relationship(
        "Permission", uselist=False, cascade="all, delete-orphan"
    )
    upgrade_metadata = relationship(
        "UpgradeMetadata", uselist=False, cascade="all, delete-orphan"
    )
    security_findings = relationship("SecurityFinding", cascade="all, delete-orphan")
    verification_results = relationship(
        "VerificationResult",
        cascade="all, delete-orphan",
        foreign_keys="VerificationResult.package_version_id",
    )
    latest_verification_result = relationship(
        "VerificationResult",
        foreign_keys=[latest_verification_result_id],
        post_update=True,
        uselist=False,
    )
...
class Capability(Base, UUIDPrimaryKeyMixin):
    ...
    capability_id = Column(
        Text,
        ForeignKey("capability_taxonomy.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    name = Column(Text, nullable=False)
    ...
    entrypoint = Column(
        Text, nullable=True
    )  # v0.2: per-tool entrypoint (module.path:function)
...
class Permission(Base, UUIDPrimaryKeyMixin):
    ...
    code_execution_level = Column(
        Enum(
            "none", "limited_subprocess", "shell", name="exec_level", create_type=False
        ),
        nullable=False,
        default="none",
    )
