...
Installed pdf-reader-pack@1.2.0`}</CodeBlock>
            ...
            <CodeBlock title="agent.py" language="python">{`from pdf_reader_pack.tool import run

# Every pack exports a run() function with a consistent interface
result = run({
    "file_path": "quarterly-report.pdf"
})

print(result["text"])
print(f"Extracted {len(result['pages'])} pages")`}</CodeBlock>
...
packages:
  ...
  web-search-pack:
    ...
    installed_at: "2025-01-15T10:31:00Z"`}</CodeBlock>

            <SubHeading>Using packs in code</SubHeading>
            <p className="mb-3 text-sm text-muted">
              Every pack exports a <C>run()</C> function from its tool module.
              This is the universal interface -- regardless of the pack, the
              pattern is always the same.
            </p>
            <CodeBlock title="agent.py" language="python">{`# Pattern: from <pack_name>.tool import run
from pdf_reader_pack.tool import run
from web_search_pack.tool import run as web_search
from email_drafter_pack.tool import run as draft_email

# Each run() accepts a dict of inputs and returns a dict of outputs
pdf_result = run({
    "file_path": "report.pdf"
})

search_result = web_search({
    "query": "AgentNode package registry",
    "max_results": 5
})
...
email_result = draft_email({
    ...
    "body_prompt": "Summarize the attached PDF findings"
})`}</CodeBlock>
...
Installed packages:
  ...
  web-search-pack    v1.0.0  trusted`}</CodeBlock>
          ...
          <section>
            ...
            <p className="mb-3 text-sm text-muted">
              ...
              Python packaging, and the tool module with a{" "}
              <C>run()</C> function.
            </p>
  ...
  src/
    my_pack/
      __init__.py
      tool.py             # Exports run() function (required)`}</CodeBlock>
            ...
            <CodeBlock title="agentnode.yaml" language="yaml">{`manifest_version: "0.1"
    ...
    - name: "manage_github"
      ...
      input_schema:
        ...
        properties:
          ...
          operation:
            type: "string"
            enum: ["create_issue", "list_issues", "review_pr", "list_repos"]
          repo:
            type: "string"
...
tags: ["github", "integration", "devtools", "automation"]`}</CodeBlock>
            ...
            <SubHeading>Step 4: Implement the run() function</SubHeading>
            <CodeBlock title="src/github_integration_pack/tool.py" language="python">{`def run(inputs: dict) -> dict:
    """
    Main entry point for the GitHub Integration Pack.

    Args:
        inputs: Dictionary with keys:
            - token (str): GitHub personal access token
            - operation (str): One of create_issue, list_issues, review_pr, list_repos
    ...
    if operation == "list_repos":
        return _list_repos(token)
    elif operation == "create_issue":
        return _create_issue(token, repo, inputs)
    elif operation == "list_issues":
        return _list_issues(token, repo)
    elif operation == "review_pr":
        return _review_pr(token, repo, inputs)
    else:
        return {"error": f"Unknown operation: {operation}"}`}</CodeBlock>
...
Published! https://agentnode.net/packages/github-integration-pack`}</CodeBlock>
          ...
          <section>
            ...
            <DocTable
              ...
              rows={[
                ["manifest_version", "string", "Yes", "ANP format version. Currently \"0.1\"."],
            ...
            <DocTable
              ...
              rows={[
                ...
                ["entrypoint", "string", "Yes", "Python module path to the tool module. Example: \"pdf_reader_pack.tool\""],
