"""The external run: the client half of the two-machine matrix, on channels that know.

This is the second verification tool. The first is frozen in `agentnode_sdk.tools.
external_run` and refuses to start; `EM3C-E6-RECORD-0001` found it confirming a sentinel by
searching the gateway's log file for a sandbox job's standard output, which is not written
there, and reporting a failure about its own search. It had been wrong in three runs.

What is carried over, deliberately: the steps of the matrix, which were never the defect and
were reviewed as they stand; the closed configuration and native start of `config` and
`launcher`; and the byte-preserving transport of `transport`. What is replaced outright is
how a crossing is confirmed -- see `crossings` below and `sentinels`.

The habits the earlier runs paid for are unchanged:

* Nothing raises. A missing executable and a timeout are outcomes.
* A step's exit code is an observation. `observed()` records what was seen, never what was
  hoped.
* Remote work is one command per step, so the recorded status is that command's own.
* Absence is concluded only from a query that succeeded and was read.
* The operator-policy binding is captured before and after and compared.
"""
...
# Used where a module is used -- at the top -- because the aliases below run when this file
# is imported, not when something calls into it.
from agentnode_sdk.verification import transport
...
from agentnode_sdk.verification import TOOL, channels, config     # noqa: E402
from agentnode_sdk.verification import sentinels                  # noqa: E402
...
#: The transport is its own module now, and this file uses it rather than restating it.
WIRE = transport.WIRE
MARKER = transport.MARKER
decode = transport.decode
launch = transport.launch
path_like = transport.path_like
one_command = transport.one_command
...
def crossings():
    """Two values crossing between the machines, each confirmed where it actually is.

    `EM3C-E6-RECORD-0001`: the previous tool confirmed both of these by opening an ssh session and
    grepping the gateway's log file. A sandbox job's output is not written there, so the answer
    was always nothing, and the tool reported that as "the value did not cross".

    Here the gateway is asked about the run, over its own transport, and answers with a signed
    record the production client verifies -- which is where a sandbox job's output is. The far
    ...
    rec.record(step(
        name="a value made here is in the far side's signed record of the run",
        role="client", argv=evidence.redact_argv([AN, "remote", "run", "crossing.py"]),
    ...
    rec.record(step(
        ...
        run_id=run_id, **asked, crossing=theirs.as_dict()))

    observed("two machines, from two channels neither of which vouches for itself",
             ["(the two crossings above)"], held, why + chr(10), role="client")
