"""report_generator_agent — AgentNode agent (ANP v0.2)

Report Generator Agent: Transform raw data (CSV, JSON) into a formatted report with charts, tables, and executive summary.
"""
...
class ReportGeneratorAgent:
    """
    Generate a business report from raw data by analyzing datasets, creating visualizations, computing statistics, and writing an executive summary.
...
async def run(goal: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    """
    ctx = context or {}
    agent = ReportGeneratorAgent(api_key=ctx.get("api_key"))
