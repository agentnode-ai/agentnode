"""data_pipeline_agent — AgentNode agent (ANP v0.2)

Data Pipeline Agent: Build and run a data pipeline: load from CSV/JSON, clean and transform, then output to a target format.
"""
...
class DataPipelineAgent:
    """
    Execute a data pipeline by loading data from source files, cleaning and transforming it according to rules, and writing the results to the target format or database.
...
async def run(goal: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    """
    ctx = context or {}
    agent = DataPipelineAgent(api_key=ctx.get("api_key"))
