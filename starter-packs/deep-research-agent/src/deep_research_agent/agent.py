"""deep_research_agent — AgentNode agent (ANP v0.2)

Deep Research Agent: Conduct deep multi-source research on any topic, synthesize findings from web and documents into a structured report.
"""
...
class DeepResearchAgent:
    """
    Research a topic thoroughly using web search, webpage extraction, and document summarization, then produce a structured report with sources and key findings.
...
async def run(goal: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    """
    ctx = context or {}
    agent = DeepResearchAgent(api_key=ctx.get("api_key"))
