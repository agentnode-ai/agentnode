"""api_design_agent — AgentNode agent (ANP v0.2)

API Design Agent: Generate an OpenAPI specification from requirements, validate it, and produce API documentation.
"""
...
class ApiDesignAgent:
    """
    Design an API by converting requirements into an OpenAPI spec, validating the schema, generating documentation with examples, and checking for REST best practices.
...
async def run(goal: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    """
    ctx = context or {}
    agent = ApiDesignAgent(api_key=ctx.get("api_key"))
