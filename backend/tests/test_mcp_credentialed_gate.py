...
_VEC = json.loads(
    (Path(__file__).resolve().parents[2] / "sdk" / "tests" / "data"
     / "allowed_domains_vectors.json").read_text(encoding="utf-8")
)
...
NPM_FULL = {"env_keys": ["TOKEN"], "install": INSTALL,
            "allowed_domains": ["api.example.com"], "npm_package": "@scope/some-mcp"}
PYPI_FULL = {"env_keys": ["TOKEN"], "install": INSTALL_PYPI,
             "allowed_domains": ["api.example.com"], "pypi_package": "some-mcp"}
...
def test_gate_noncredentialed_validates_domains_if_present():
    assert check_credentialed_publish({"allowed_domains": ["http://evil"]})  # has errors
...
def test_gate_credentialed_requires_install_and_domains():
    ...
    assert check_credentialed_publish({"env_keys": ["TOKEN"], "install": INSTALL})  # missing domains
    assert check_credentialed_publish(
        {"env_keys": ["TOKEN"], "allowed_domains": ["api.x.com"]})  # missing install
...
def test_binding_pypi_install_with_divergent_npm_package():
    assert any("npm_package" in e
               for e in check_credentialed_publish(dict(PYPI_FULL, npm_package="@scope/some-mcp")))
...
async def test_verify_registry_blocks_credentialed_without_domains():
    sv = await verify_registry({"mcp_server": {
        "env_keys": ["TOKEN"], "install": INSTALL, "npm_package": "@scope/some-mcp",
...
async def test_verify_registry_blocks_credentialed_without_install():
    sv = await verify_registry({"mcp_server": {
        "env_keys": ["TOKEN"], "allowed_domains": ["api.example.com"],
        "npm_package": "@scope/some-mcp", "command": ["npx", "-y", "@scope/some-mcp@1.2.3"]}})
    assert sv["server_status"] == "mismatch"
...
async def test_verify_registry_blocks_install_package_mismatch():
    ...
    sv = await verify_registry({"mcp_server": dict(NPM_FULL, npm_package="@scope/evil")})
    assert sv["server_status"] == "mismatch"
