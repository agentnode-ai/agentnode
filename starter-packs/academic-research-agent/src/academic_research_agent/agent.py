"""academic_research_agent — AgentNode agent (ANP v0.2)

Academic Research Agent: Search academic papers on arXiv and Google Scholar, extract citations, and produce a literature review.
"""
...
class AcademicResearchAgent:
    """
    Conduct academic literature research by searching paper databases, extracting PDFs, collecting citations, and summarizing findings into a literature review.
...
async def run(goal: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    """
    ctx = context or {}
    agent = AcademicResearchAgent(api_key=ctx.get("api_key"))
