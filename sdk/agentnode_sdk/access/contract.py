...
OPERATIONS = (
    ...
    Operation(
        name="connections.enrol",
        since="2",
        needs=MANAGE_DEVICES,
        audience=PERSON,
        risk=CHANGES_ACCESS,
        confirms_with_a_person=True,
        summary="Begin setting up a new AI connection, and issue the challenge it must answer.",
        params=(
            Field("way_in", "string", "the channel that connection will use", one_of=CHANNELS),
            Field("label", "string", "what to call it, in words a person would recognise"),
        ),
        returns=(
            Field("challenge", "string", "what to ask about while waiting"),
            Field("ticket", "string",
                  "one download of the setup file; the credential is created when it is "
                  "collected and never passes through the page"),
            Field("expires_at", "integer", "when this stops being offered"),
        ),
        errors=COMMON,
        changes=True,
    ),
    Operation(
        ...
        risk=READS,
        confirms_with_a_person=False,
        summary="Whether that connection has actually carried out the operation it was asked to.",
        params=(Field("challenge", "string", "the challenge to ask about"),),
        returns=(
            Field("satisfied", "boolean",
                  "true only when this gateway recorded the expected call from that exact "
                  "connection, over that exact channel, after this challenge was issued"),
            Field("label", "string", "the connection being set up"),
            Field("way_in", "string", "the channel it was set up for"),
            Field("operation", "string", "what it has to do to be believed"),
            Field("why", "string", "what is still missing, when it is not satisfied",
                  required=False),
        ),
        errors=COMMON + ("no_such_run",),
    ),
    Operation(
        ...
        risk=READS,
        confirms_with_a_person=False,
        summary="The browser sessions open on this account, and when each was last used.",
        params=(),
        returns=(Field("sessions", "array",
                       "each session, by the name this gateway gave it -- never by anything "
                       "that could be presented as one"),),
        errors=COMMON,
    ),
    Operation(
        name="sessions.end",
        ...
        risk=CHANGES_ACCESS,
        confirms_with_a_person=True,
        summary="End a browser session, so it stops working at once.",
        params=(Field("session", "string",
                      "the session to end, by the name the list shows; omit to end this one",
                      required=False),),
        returns=(
            Field("session", "string", "the session ended"),
