"""spreadsheet_auditor_agent — AgentNode agent (ANP v0.2)

Spreadsheet Auditor Agent: Audit Excel/CSV spreadsheets for errors, duplicates, formula issues, and data inconsistencies.
"""
...
class SpreadsheetAuditorAgent:
    """
    Audit a spreadsheet by checking for duplicates, data type mismatches, formula errors, missing values, and inconsistencies, then produce an audit report with fix suggestions.
...
async def run(goal: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    """
    ctx = context or {}
    agent = SpreadsheetAuditorAgent(api_key=ctx.get("api_key"))
