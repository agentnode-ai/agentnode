...
export interface PackageSearchProps {
  /** Filters locked by the embedding page (e.g. {package_type:"skill"} or {runtime:"mcp"}). Locked keys are applied to every query, hidden from the sidebar, and kept out of the URL. */
  fixed?: Partial<Filters>;
  /** URL path to sync query params to. */
  basePath?: string;
...
function SearchContent({ fixed = {}, basePath = "/search", heading = "Search Packages", autoFocus = true }: PackageSearchProps) {
  ...
  const performSearch = useCallback(
    async (q: string, f: Filters, p: number, sort: string) => {
      ...
      if (q) params.set("q", q);
      // Locked (fixed) keys stay out of the URL — they are implied by the page.
      (Object.keys(f) as (keyof Filters)[]).forEach((k) => {
        if (f[k] && !fixed[k]) params.set(k, f[k]);
      });
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6 py-12">
      ...
      <div className="mb-8">
        {heading !== null && (
          <h1 className="mb-4 text-3xl font-bold text-foreground">{heading}</h1>
        )}
        <SearchInput
          value={query}
          onChange={setQuery}
...
export default function PackageSearch(props: PackageSearchProps) {
  return (
    <Suspense fallback={<SearchFallback heading={props.heading} />}>
      <SearchContent {...props} />
    </Suspense>
  );
}
