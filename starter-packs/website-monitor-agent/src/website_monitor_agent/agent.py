"""website_monitor_agent — AgentNode agent (ANP v0.2)

Website Monitor Agent: Monitor websites for content changes, downtime, and visual regressions, and send alerts via Slack or email.
"""
...
class WebsiteMonitorAgent:
    """
    Monitor target websites by periodically checking for content changes, downtime, and visual regressions, then send alerts with diff details via configured channels.
...
async def run(goal: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    """
    ctx = context or {}
    agent = WebsiteMonitorAgent(api_key=ctx.get("api_key"))
